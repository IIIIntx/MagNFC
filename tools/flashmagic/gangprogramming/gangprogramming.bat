...
echo.
echo Prerequisites:
echo   Files SWDLINK2.hex and SWDLPC11U35.bin are copied from the installation
echo   folder of Flash Magic to the Interfaces subfolder here.
echo.
...
::######################################################################################################################
    ...
    :checkPrerequisites
    if not exist "Interfaces\SWDLINK2.hex" (
        echo.
		echo Prerequisites are not met.
		echo.
		goto :eof
	)
    if not exist "Interfaces\SWDLPC11U35.bin" (
        echo.
		echo Prerequisites are not met.
		echo.
		goto :eof
	...
	::Display Manual at the beginning if no argument is provided
	:displayManual
	echo Flash Magic Gang Programming
	echo.
	echo User Manual:
