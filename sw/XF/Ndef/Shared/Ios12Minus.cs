...
namespace Plugin.Ndef
{
    ...
    internal class Ios12Minus : NSObject, INdef, INFCNdefReaderSessionDelegate
    {
        ...
        public async Task DeInitTagReaderAsync()
        {
            if (_session != null)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    Debug.WriteLine("OA Caught exception:" + _tagIdStr + " - " + Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
                }
                finally
                {
        ...
        public async Task<(Status status, List<NdefRecord> rdNdefRecords)> ReadAsync()
        {
            ...
            try
            {
                for (int i = 0; i < 3; i++)
                {
                    ...
                    (Status rxStat, NdefMessage rdNdefMessage) = await ReadNdefMessageAsync();
                    switch (rxStat)
                    {
            ...
            {
                Helpers.ExceptionLogHelper.Log(_tagIdStr, Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
                Debug.WriteLine("OB Caught exception:" + _tagIdStr + " - " + Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
            }
            finally
            {
                _sem.Release();
        ...
        public void DidDetect(NFCNdefReaderSession session, NFCNdefMessage[] messages)
        {
            ...
            if (_isNewRead)
            {
                Debug.WriteLine($"{DateTime.Now.TimeOfDay}" + "#### DidDetect DATA:" + BitConverter.ToString(msg.ToByteArray()).Replace("-", string.Empty));
        ...
        private async Task<(Status status, NdefMessage mgs)> ReadNdefMessageAsync()
        {
            ...
            Helpers.TimerHelper timer = null;
            ...
            try
            {
                ...
                if (_session != null)
                {
                    ...
                    timer = new Helpers.TimerHelper(TimeSpan.FromMilliseconds(1000), () =>
                    {
                        ...
                        {
                            Helpers.ExceptionLogHelper.Log(_tagIdStr, Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
                            Debug.WriteLine("OC Caught exception:" + _tagIdStr + " - " + Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
                        }
                    });
                }
                else
            ...
            {
                Helpers.ExceptionLogHelper.Log(_tagIdStr, Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
                Debug.WriteLine("OD Caught exception:" + _tagIdStr + " - " + Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
            ...
            {
                Helpers.ExceptionLogHelper.Log(_tagIdStr, Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
                Debug.WriteLine("OE Caught exception:" + _tagIdStr + " - " + Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
                status = Status.TagReadFailed;
            }
            finally
            {
