...
namespace Plugin.Ndef
{
    ...
    internal class Ios13Plus : NFCTagReaderSessionDelegate, INdef
    {
        ...
        public override async void DidDetectTags(NFCTagReaderSession session, INFCTag[] tags)
        {
            ...
            try
            {
                ...
                if (tag.Type == (NFCTagType)4) // NFCTag.MiFare
                {
                    ...
                    // Read NDEF message.
                    (Status status, List<NdefRecord> ndefRecords) = await ReadAsync();
                    if (status != Status.OK)
                    {
                    ...
                    foreach (NdefRecord record in ndefRecords)
                    {
            ...
            catch (Exception ex)
            {
                ...
                Debug.WriteLine("ON Caught exception:" + _tagIdStr + " - " + Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
        ...
        {
            Status status = await WriteAsync(wrNdefRecords);
            for (int i = 0; i < 3; i++)
            {
                if (status != Status.OK)
                {
                    await Task.Delay(10);
                    status = await WriteAsync(wrNdefRecords);
                }
            }
            if (status != Status.OK)
            {
        ...
        public async Task<(Status status, List<NdefRecord> rdNdefRecords)> ReadAsync()
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                {
                    NdefMessage rdNdefMessage = new NdefMessage();
                    rdNdefMessage.AddRange(ret.rdNdefRecords);
                    byte[] rdNdefMessageBytes = rdNdefMessage.ToByteArray();
                    ...
                    {
                        /* No NDEF read or read our own NDEF. Retry. */
            ...
            async Task<(Status status, List<NdefRecord> rdNdefRecords)> ReadNdefAsync()
            {
                try
                {
                    TaskCompletionSource<(Status, List<NdefRecord>)> tcs = new TaskCompletionSource<(Status, List<NdefRecord>)>();
                    _tag.ReadNdef((iosNdefMessage, error) =>
                    {
                        ...
                        {
                            Helpers.ExceptionLogHelper.Log(_tagIdStr, Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
                            Debug.WriteLine("OO Caught exception:" + _tagIdStr + " - " + Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
                ...
                {
                    Helpers.ExceptionLogHelper.Log(_tagIdStr, Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
                    Debug.WriteLine("OP Caught exception:" + _tagIdStr + " - " + Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
        ...
        {
            NFCNdefPayload[] iosNdefRecords = new NFCNdefPayload[wrNdefRecords.Count];
            for (int i = 0; i < wrNdefRecords.Count; i++)
            {
                byte[] id = wrNdefRecords[i].Id == null ? new byte[] { } : wrNdefRecords[i].Id;
            ...
            NFCNdefMessage iosNdefMessage = new NFCNdefMessage(iosNdefRecords);

            NdefMessage wrNdefMessage = new NdefMessage();
            ...
            if (_tag != null)
            {
                try
                {
                    TaskCompletionSource<Status> tcs = new TaskCompletionSource<Status>();
                    _tag.WriteNdef(iosNdefMessage, error =>
                    {
                        ...
                        {
                            Helpers.ExceptionLogHelper.Log(_tagIdStr, Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
                            Debug.WriteLine("OQ Caught exception:" + _tagIdStr + " - " + Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
                            tcs.SetResult(Status.TagWriteFailed);
                        }
                    });
                    _sem.Release();
                    Status status = await tcs.Task;
                    return status;
                }
                catch(Exception ex) 
                {
                    Helpers.ExceptionLogHelper.Log(_tagIdStr, Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
                    Debug.WriteLine("OR Caught exception:" + _tagIdStr + " - " + Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
                }
            }
            _sem.Release();
