...
namespace Plugin.Ndef
{
    internal class PcscDevice : INdef, IDisposable
    {
        ...
        internal PcscDevice()
        {
            IDeviceMonitorFactory deviceFactory = DeviceMonitorFactory.Instance;
        ...
        public async Task InitTagReaderAsync(TagReaderStatusChanged onTagReaderStatus)
        {
            ...
            catch (Exception ex)
            {
                ...
                Debug.WriteLine("PH Caught exception:" + _tagIdStr + " - " + Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
            }
        }
            ...
            List<NdefRecord> wrNdefRecords)
        {
            ...
            try
            {
                Status txStat = WriteNdefMessage(txNdefMsg);
                if (txStat != Status.OK)
                {
                    return (txStat, null);
                }
                ...
                for (int i = 0; i < 5; i++)
                {
                    ...
                    (Status rxStat, NdefMessage rxNdefMsg) = ReadNdefMessage();
                    switch (rxStat)
                    {
                    ...
                    {
                        List<NdefRecord> rdNdefRecords = rxNdefMsg.ToList();
            ...
            {
                Helpers.ExceptionLogHelper.Log(_tagIdStr, Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
                Debug.WriteLine("PA Caught exception:" + _tagIdStr + " - " + Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
            }
            finally
            {
                sem.Release();
        ...
        public async Task<(Status status, List<NdefRecord> rdNdefRecords)>ReadAsync()
        {
            ...
            {
                Helpers.ExceptionLogHelper.Log(_tagIdStr, Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
                Debug.WriteLine("PB Caught exception:" + _tagIdStr + " - " + Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
            }
            finally
            {
                sem.Release();
        ...
        {
            NdefMessage txNdefMsg = new NdefMessage();
            txNdefMsg.AddRange(wrNdefRecords);
            ...
            {
                Status txStat = WriteNdefMessage(txNdefMsg);
                return (txStat != Status.OK) ? txStat : Status.TagReadFailed;
            }
            catch (Exception ex)
            {
                Helpers.ExceptionLogHelper.Log(_tagIdStr, Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
                Debug.WriteLine("PG Caught exception:" + _tagIdStr + " - " + Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
            }
            finally
            {
                sem.Release();
        ...
        (Status status, NdefMessage mgs) ReadNdefMessage()
        {
            ...
            try
            {
                ...
                byte[] ndef = new byte[ndefTlvLen];
                Buffer.BlockCopy(response, payloadOffset, ndef, 0, ndefTlvLen);
                Debug.WriteLine($"{DateTime.Now.TimeOfDay}" + $"#### READ:" + BitConverter.ToString(ndef).Replace("-", string.Empty));
            ...
            {
                Helpers.ExceptionLogHelper.Log(_tagIdStr, Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
                Debug.WriteLine("PC Caught exception:" + _tagIdStr + " - " + Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
                if (stat == Status.OK)
                {
        ...
        Status WriteNdefMessage(NdefMessage msg)
        {
            ...
            try
            {
                ...
                Debug.WriteLine($"{DateTime.Now.TimeOfDay}" + $"####WRITE:" + BitConverter.ToString(msg.ToByteArray()).Replace("-", string.Empty));
            ...
            {
                Helpers.ExceptionLogHelper.Log(_tagIdStr, Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
                Debug.WriteLine("PD Caught exception:" + _tagIdStr + " - " + Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
                if (stat == Status.OK)
                {
        ...
        void OnCardInserted(object sender, CardStatusEventArgs e)
        {
            ...
            try
            {
                ...
                byte[] uid0 = ReadBinary(0, 0, 4);
                byte[] uid1 = ReadBinary(0, 1, 4);
                _tagId = new byte[]
                {
                ...
                (Status stat, NdefMessage msg) = ReadNdefMessage();
                if (stat == Status.OK)
                {
            ...
            {
                Helpers.ExceptionLogHelper.Log(_tagIdStr, Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
                Debug.WriteLine("PE Caught exception:" + _tagIdStr + " - " + Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
            }
        }
        ...
        void OnCardRemoved(object sender, CardStatusEventArgs e)
        {
            try
            {
                ...
                {
                    if (readerContext.isoReader != null)
                    {
            ...
            {
                Helpers.ExceptionLogHelper.Log(_tagIdStr, Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
                Debug.WriteLine("PF Caught exception:" + _tagIdStr + " - " + Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
            }
        ...
        {
            foreach (string removed in e.DetachedReaders)
            {
                RemoveReader(removed);
                Debug.WriteLine($"Reader detached: {removed}");
            }

            foreach (string added in e.AttachedReaders)
            {
                AddReader(added);
        ...
        byte[] ReadBinary(byte msb, byte lsb, int size)
        {
            ...
            unchecked
            {
                ...
                Response response = readerContext.isoReader.Transmit(readBinaryCmd);
        ...
        public void UpdateBinary(byte msb, byte lsb, byte[] data)
        {
            ...
            Response response = readerContext.isoReader.Transmit(updateBinaryCmd);
