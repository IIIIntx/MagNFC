...
namespace Msg
{
    ...
    public class CommandHandler
    {
        ...
        private async Task<(bool isOk, int numData, bool isMoreEventData, int lastEventIndex)> SendCmdGetRspAsync(List<NdefRecord> cmdList)
        {
            ...
            try
            {
                List<NdefRecord> rspList;
                Plugin.Ndef.Status status;
                (status, rspList) = await _ndef.WriteReadAsync(cmdList);
                if (status == Plugin.Ndef.Status.OK)
                {
                    NotifyCompletedWriteAction(cmdList);
                    NotifyCompletedReadAction(rspList);
                    (isOk, numData, isMoreEventData, lastEventIndex) = CheckResponses(rspList);

                    if (!isOk)
                    {
                        /* No write, attempt a second read. */
                        await Task.Delay(10);
                        (status, rspList) = await _ndef.ReadAsync();
                        if (status == Plugin.Ndef.Status.OK)
                        {
                            NotifyCompletedReadAction(rspList);
                            (isOk, numData, isMoreEventData, lastEventIndex) = CheckResponses(rspList);
            ...
            {
                Helpers.ExceptionLogHelper.Log(_lib.TagModel.TagId, Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
                Debug.WriteLine("Caught exception:" + _lib.TagModel.TagId + " - " + Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
                isOk = false;
            }

            return (isOk, numData, isMoreEventData, lastEventIndex);
        }

        private (bool isOk, int numData, bool isMoreEventData, int lastEventIndex) CheckResponses(List<NdefRecord> rspList)
        {
            bool isOk = true;
            int numData = 0;
            bool isMoreEventData = false;
            ...
            {
                ResponseHandler rspHandler = new ResponseHandler(_lib, rsp);
                isOk &= !rspHandler.IsError;
        ...
        private void NotifyCompletedWriteAction(List<NdefRecord> cmdList)
        {
            NdefMessage ndefMessage = new NdefMessage();
            ndefMessage.AddRange(cmdList);
            byte[] data = ndefMessage.ToByteArray();
        ...
        private void NotifyCompletedReadAction(List<NdefRecord> rspList)
        {
            NdefMessage ndefMessage = new NdefMessage();
