...
namespace Helpers
{
    public class RtcAccuracyHelper
    {
        ...
        static public void CalculateCorrectionFactor(DateTime initTimestamp, DateTime finalTimestamp)
        {
            ...
            {
                double worldSec = DateTime.UtcNow.Subtract(initTimestamp).TotalSeconds;
                ulong islandSec = (ulong)finalTimestamp.Subtract(initTimestamp).TotalSeconds;
        ...
        static public DateTime CorrectedTimestamp(DateTime timestamp)
        {
            ...
            {
                double delta = timestamp.Subtract(_initTimestamp).TotalSeconds * _correctionFactor;
        ...
        {
            DateTime correctedLocalTimestamp = CorrectedTimestamp(timestamp);
        ...
        static public DateTime CorrectedTimestamp(DateTime initTimestamp, DateTime timestamp, double correctionFactor)
        {
            ...
            {
                double delta = timestamp.Subtract(initTimestamp).TotalSeconds * _correctionFactor;
        ...
        {
            DateTime correctedLocalTimestamp = CorrectedTimestamp(initTimestamp, timestamp, correctionFactor);
            return correctedLocalTimestamp.ToLocalTime();
        }

        static public TimeSpan CorrectedTimeSpan(TimeSpan timeSpan)
        {
            TimeSpan correctedTimeSpan = timeSpan;
            double delta = correctedTimeSpan.TotalSeconds * _correctionFactor;
