...
namespace Helpers
{
    public class SvgToPngHelper
    {
        ...
        public (byte[] pngData, string filteredSvgData) Convert(string svgData, Func<string, byte[]> readFile)
        {
            ...
            string filteredSvgData = svgData;

            //MatchCollection text = Regex.Matches(svgData, @"<text\s*(.+?)\s*</text>");

            // Draw svg. 
            SkiaSharp.Extended.Svg.SKSvg svg = new SkiaSharp.Extended.Svg.SKSvg();
            SKPicture picture = svg.Load(new MemoryStream(Encoding.ASCII.GetBytes(svgData)));
            SKSizeI dimension = new SKSizeI((int)Math.Ceiling(picture.CullRect.Width), (int)Math.Ceiling(picture.CullRect.Height));
            SKBitmap bitmap = new SKBitmap(dimension.Width, dimension.Height);
            ...
            MatchCollection icons = Regex.Matches(svgData, @"<image\s*(.+?)\s*/>");
            for (int i = 0; i < icons.Count; i++)
            {
                try
                {
                    Match xStr = Regex.Match(icons[i].Value, @"x\s*=\s*(.+?)""");
                    if (!xStr.Success)
                    {
                    ...
                    Match yStr = Regex.Match(icons[i].Value, @"y\s*=\s*(.+?)""");
                    if (!yStr.Success)
                    {
                    ...
                    Match widthStr = Regex.Match(icons[i].Value, @"width\s*=\s*(.+?)""");
                    if (!widthStr.Success)
                    {
                    ...
                    Match heightStr = Regex.Match(icons[i].Value, @"height\s*=\s*(.+?)""");
                    ...
                    Match iconStr = Regex.Match(icons[i].Value, @"xlink:href\s*=\s*(.+?)""");
                    if (!iconStr.Success)
                    {
                    ...
                    SKImage iconImg = SKImage.FromBitmap(SKBitmap.Decode(readFile(icon)));
