...
namespace TLogger.Services
{
    public class DatabaseService
    {
        ...
        private async void OnNdefAccess(object sender, Lib.NdefAccessEventArgs e)
        {
            if (App.AppSettingsService.AreDebugLogs)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    ExceptionLogHelper.Log(tagId, Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
                    Debug.WriteLine("DA Caught exception:" + tagId + " - " + ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
                }
                finally
                {
                    _sem.Release();
        ...
        private async void OnExceptionLogEvent(object sender, ExceptionLogEventArgs e)
        {
            if (App.AppSettingsService.AreDebugLogs)
            {
                ...
                catch (Exception ex)
                {
                    var tagId = _msgLib.TagModel.TagId;
                    ExceptionLogHelper.Log(tagId, Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
                    Debug.WriteLine("DB Caught exception:" + tagId + " - " + ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
                }
                finally
                {
                    _sem.Release();
        ...
        private async void OnDataProcess(object sender, Lib.DataProcessEventArgs e)
        {
            ...
            {
                ExceptionLogHelper.Log(tagId, Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
                Debug.WriteLine("DC Caught exception:" + tagId + " - " + ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
            }
            finally
            {
                _sem.Release();
        ...
        private async void OnSetConfig(object sender, Lib.SetConfigEventArgs e)
        {
            ...
            {
                ExceptionLogHelper.Log(tagId, Helpers.ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
                Debug.WriteLine("DD Caught exception:" + tagId + " - " + ExceptionLogHelper.GetCurrentMethod() + " - " + ex.Message);
            }
            finally
            {
                _sem.Release();
